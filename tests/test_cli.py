...
def test_format_dir_structure_basic(simple_structure: Path):
    ...
    result = format_dir_structure(simple_structure, exclude_patterns=set(), include_patterns=set())
...
def test_format_dir_structure_with_exclusions(project_structure: Path):
    ...
    result = format_dir_structure(project_structure, exclude_patterns=exclude, include_patterns=set())
...
def test_format_dir_structure_empty_directory(tmp_path: Path):
    ...
    result = format_dir_structure(empty_dir, exclude_patterns=set(), include_patterns=set())
...
def test_main_with_unlimited_depth(nested_structure: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sys.argv", ["indastructa", str(nested_structure)]
    )
...
def test_main_with_include_overrides_default_exclusion(project_structure: Path, monkeypatch):
...
def test_main_with_file_instead_of_directory(project_structure: Path, monkeypatch, capsys):
    ...
    captured = capsys.readouterr()
    assert "not a directory" in captured.err.lower() or "must be a directory" in captured.err.lower()
...
def test_output_has_header(project_structure: Path, monkeypatch):
    ...
    assert any(keyword in lines[0].lower() for keyword in ["project", "structure", "---"])
